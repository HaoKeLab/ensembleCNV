...
sub splitIlluminaReport {
	...
	my ($count_line, $count_file, $pre, $name_index, $sample_index, $lrr_index, $baf_index, $chr_index, $position_index) = (0, 0, 'NA');
	my (@field);
	...
	@field >= 6 or confess "Error: invalid header line (at least 6 tab-delimited fields, including 'SNP Name', 'Sample ID', 'Chr', 'Position', 'B Allele Freq', 'Log R Ratio' expected) in report file $reportfile: <$_>\n";
	...
	while (<REPORT>) {
		...
		if ($field[$sample_index] eq $pre) {
			...
			#if ( $revised_file ) {
			#	print OUT "Name\tChr\tPosition\tLog R Ratio\tB Allele Freq\n";
			#}
			#else {
			#	print OUT "Name\tChr\tPosition\tLog R Ratio\tB Allele Freq\n";
			#}
...
'SNP Name', 'Sample ID', 'Chr', 'Position', 'B Allele Freq', 'Log R Ratio', 
'Allele1 - Forward', 'Allele1 - Forward',  'X', 'Y', (To run QuantiSNP, only the first 
6 fields are needed, but the last 4 are required to run iPattern). If not, try to generate 
...
'SNP Name', 'Sample ID', 'Chr', 'Position', B Allele Freq', 'Log R Ratio' fields will be examined 
