...
pipeline_main <- function(dt_cnvrs, paras_LRR, dup_pairs, samples_LRR,
  ...
  png(filename = paste0('./laptop_check/res/', file_diagnosis), width = 12, height = 12, units = "in", res = 512)
  plot_gmm_diagnosis(dt_cnvr = dt_cnvr_train, paras_model = paras_model)
  dev.off()
  
  # set CN = 0 
  mu0 <- -3
  # sigma0 <- NULL  ## MUST BE CHANGED
  sigma0 = 0.8*0.8
  if (n0 != 0) {
    if (n0  >= 5) {
  ...
  png(filename = paste0('./laptop_check/res/', file_steps), width = 12, height = 12, units = "in", res = 512)
  ...
  call_rate <- length(idxs_nocall)/nrow(res_gatk_pred_final)
  ...
  plot_gatk <- ggplot() + 
    ...
    ggtitle(label = "CNV call from gatk similary method",
            subtitle = paste("GQ score:", GQ_score, "call rate:", round(call_rate, 2))) 
  
  plot_gatk
  
  filefinal <- paste0("summary_", cnvr_id, ".png")
  png(filename = file.path('./laptop_check/res/', filefinal),
...
# filename <- "C:/mssm_projects/STARNET/work_3/Predict_All_CNVR/model_test_numSNP/res_CNVR_with_CN1/dat_CNVR_check/CNVR_107_r1_chr1_p.rds"
filename <- "C:/mssm_projects/STARNET/work_3/Predict_All_CNVR/model_test_numSNP/functions_gatk/laptop_check/dat/CNVR_51_r1_chr22_q.rds"
...
# test CNVR_ID: CNVR_107_r1_chr1_p.rds
# CNVR_105_r1_chr1_p
# CNVR_104_r1_chr1_p
