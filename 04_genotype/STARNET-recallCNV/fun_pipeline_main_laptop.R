...
pipeline_main <- function(dt_cnvrs, paras_LRR, dup_pairs, samples_LRR,
  ...
  png(filename = paste0('./png_check/', file_diagnosis), width = 12, height = 12, units = "in", res = 512)
  ...
  if (n0 == 0) {
    ...
    model1$lambda <- c(0, paras_all$lambdas)
  } else {
    # number of CN = 1
  ...
  png(filename = paste0('./png_check/', file_steps), width = 12, height = 12, units = "in", res = 512)
  ...
  png(filename = file.path('./png_check/', filefinal),
...
filename <- "C:/mssm_projects/STARNET/work_3/Predict_All_CNVR/model_test_numSNP/functions_gatk/res_check_cnvr/CNVR_32_r1_chr20_p.rds"
dt_cnvrs <- readRDS(file = filename)
