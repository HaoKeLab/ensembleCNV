...
# raw model only with CN = 1/2/3 

library(mixtools)
library(modeest)
# =================================
# add n1/n2 freq percentage cutoff
# =================================
# change in the part 4 when n1 < 10 and n3 < 10
# ==========================
# deal with condition number of percent of CN = 2 <= 50%
# ==========================
# add paras_new
# change stage 2 gmm model
# ========================
# not train model if number of samples in the CN less 10 
# in the second round gmm

# inspired from zhongyang
# CN = 2 use mode
# CN = 1 and 3 use mean 
# cutoff of number of cn set 10 to calculate own mean and sd
...
model1_zz <- function(dt_cnvr, paras_LRR) {
  ...
  if (lambdas1[2] <= 0.5) {
    ...
    # dt3_new <- subset(dt_new, CN == 3)  # need to change
  ...
  # probs_pred <- apply(model1$posterior, MARGIN = 1, max)
  ...
  # model2 <- normalmixEM(x = dt_cnvr$LRR_median, mu = c(mu1, mu2, mu3),
  #                       sigma = c(sigma1, sigma2, sigma3), 
  #                       mean.constr = c(mu1, mu2, mu3), k = 3)
  
  # add 8_cc here
  # flag n11/n12 and n31/n32
...
model3_zz <- function(dt_cnvr, paras_LRR) {
  ...
  # res <- list(paras_all = list(mus = model1$mu,
  #                              sigmas = model1$sigma,
  #                              lambdas = model1$lambda),
  #             paras_model = paras_model)
  # return(res)  # return

  # stage1 model parameter
  paras_model$stage1$model <- list(mu = model1$mu,
                                   sigma = model1$sigma,
                                   lambda = model1$lambda)
  ...
  if (flag2) {
    ...
    #                                  lambda = model2$lambda)
    # 
    # paras_all <- list(mus = model2$mu, sigmas = model2$sigma, lambdas = model2$lambda)
...
model4_zz <- function(dt_cnvr, paras_LRR) {
  ...
  # first stage model
  # model1 <- normalmixEM(x = dt_cnvr$LRR_median, mu = c(mu1, mu2, mu3),
  #                       sigma = c(sigma1, sigma2, sigma3), lambda = c(n1, n2, n3)/n, k = 3)
  ...
  if (flag2) {
    ...
    mu2 <- paras2$mu
    sigma2 <- paras2$sigma
    # set n2 in second stage training model
    n2 <- nrow(dt22)
...
train_model_zz <- function(dt_cnvr, paras_LRR) {
  ...
  # dt_cnvr_new <- rbind(dt_cnvr, dt_cn1_permute, dt_cn3_permute)
  # dt_cnvr_new <- rbind(dt_cnvr, dt_cn3_permute)
  # paras <- model1_zz(dt_cnvr = dt_cnvr_new, paras_LRR = paras_LRR)
  # return(paras)
