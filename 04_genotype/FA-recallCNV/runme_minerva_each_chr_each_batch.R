...
# =================
# for Food_Allergy each chr each batch
# =================
# add log file
## add GQ score cutoff = 40 here
# check each cnvr's result
# all results save in the following path:

# args <- commandArgs(trailingOnly = TRUE)
# 
# chr1 <- as.integer(args[1])   # chr infor
# batch1 <- as.integer(args[2])  # batch
# type1 <- as.integer(args[3])  
# # 0 for resubmit total CNVR and 1 for running only failed CNVR
# # batch1 <- as.integer(args[2]) # which batch
# # chr1 <- 11
# # cnvr1 <- "CNVR_431_r1_chr11_p"
# 
# flag.log <- TRUE
# 
# if (type1 != 0 & type1 != 1) {
...
option_list = list(
  ...
  make_option(c("-t", "--type"), action = "store", type = "integer", default = NA,
              help = "which type (0-all, 1-failed)")
)
...
path_sourcefile = "/sc/orga/projects/haok01a/chengh04/Food_Allergy/code_batch/ensembleCNV/pipeline_regenotypeCNV_raw"
...
source(file = file.path(path_sourcefile, 'fun_models_zz_8_cc_raw_v1.R'))  ## changed here
...
# for 3 batch
# file_PennCNV = "/sc/orga/projects/haok01a/chengh04/Food_Allergy/code_batch/ensembleCNV/3batch/dat/PennCNV_sample_QC.rds"
# samples_LRR  = readRDS(file = file_PennCNV)
...
path_matrix <- "/sc/orga/projects/haok01a/chengh04/Food_Allergy/matrix_chr_batch/rds_5batch_hg19" ## hg19 transform
...
# cat("cnvr1:", cnvr1, "\n")
# snp_start <- dt_cnvrs1$start_snp[1]
# snp_end   <- dt_cnvrs1$end_snp[1]
# 
...
# idx_outer_start <- dt_cnvrs1$outer.start[1]
# idx_outer_end   <- dt_cnvrs1$outer.end[1] 
# idx_start_new <- ifelse(idx_outer_start - 20 <= 0, 1, idx_outer_start - 20)  # new start and end for plot heatmap
...
# 
# dt_PFB1 <- subset(dt_PFB, Name %in% snps_name)
# dt_cnvr_stat <- merge(dt_cnvr_stat, dt_PFB1, all.x = TRUE)  # all snps are cnv_snp
# 
# samples <- colnames(dt_matrix_BAF)
...
# 
# ## save cnvr stat data
# # path_save <- "/sc/orga/projects/haok01a/chengh04/Food_Allergy.New/Code_CNV_gatk/res/dat"
# # saveRDS(dt_cnvr_stat, file = file.path(path_save, paste0(cnvr1, ".rds")))
# 
# res_pipeline_cnvr1 <- pipeline_main(dt_cnvrs = dt_cnvr_stat, paras_LRR = paras_LRR, dup_pairs = dup_pairs,
#                                     samples_LRR = samples_LRR, path_png = path_png, GQ_score = 20)
# 
# res_gatk_pred_final <- res_pipeline_cnvr1$res_gatk_pred_final
# res_pars <- res_pipeline_cnvr1$res_pars
# 
...
# 
# # filename_pred <- paste0("CNVR_pred_chr_", chr1, "_batch_", batch1, ".rds")
# filename_pars <- paste0("CNVR_pars_", cnvr1, ".rds")
# 
# # saveRDS(res_pred_all, file = file.path(path_pred, filename_pred)) # pred file
# saveRDS(res_pars_all, file = file.path(path_pars, filename_pars)) # pars file
