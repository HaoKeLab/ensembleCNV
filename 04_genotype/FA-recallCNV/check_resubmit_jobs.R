...
## main predict path
path_main_pred <- "/sc/orga/projects/haok01a/chengh04/Food_Allergy/code_batch/ensembleCNV/5batch/res_regenotypeCNV_raw/pred"
...
check_jobs <- function(path_main, dt_cnvr_raw, flag) {
  ...
  for (i in 1:nrow(dt_freq_raw)) {
    ...
    if ( !dir.exists(paths = path1) ) { 
      ...
      if (nrow(dt1.failed) == 0) {
        ...
        path_cnvr_failed = "/sc/orga/projects/haok01a/chengh04/Food_Allergy/code_batch/ensembleCNV/5batch/res_regenotypeCNV_raw/cnvr_failed"
        if ( !dir.exists(paths = path_cnvr_failed) ) {
          dir.create(path = path_cnvr_failed, showWarnings = FALSE)
        }
        
        fname.failed <- paste0("cnvrs_chr_", chr1, "_batch_", batch1, "_failed.rds")
        saveRDS(dt1.failed, file = file.path(path_cnvr_failed, fname.failed))
...
# main run function --------------------------------------------
check_jobs(path_main = path_main_pred, dt_cnvr_raw = dt_cnvr_raw, flag = flag)
