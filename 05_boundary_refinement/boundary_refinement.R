...
# refine_step1_r = function(Yt, min.len = 5) {
# 
#   n = ncol(Yt)
#   stat1 = matrix(NA, n, n)
#   max.value = -Inf
...
# 
#   Mcorr = cor(Yt)
#   S = Mcorr[lower.tri(Mcorr)]
#   n2 = length(S)
# 
#   for (i in 1:(n-min.len+1)) {
#     for (j in (i+min.len-1):n) {
# 
#       # cat(i, j, "\n")
#       M <- Mcorr[i:j,i:j]
#       x <- M[lower.tri(M)]
#       n1 <- length(x)
#       if (n1==n2) {
#         tmp <- mean(x)/sqrt(1/n1)
#       } else {
#         tmp <- (mean(x) - (sum(S)-sum(x))/(n2-n1))/sqrt(1/n1+1/(n2-n1))
#       }
#       stat1[i,j] <- tmp
#       if (tmp>max.value) {
#         max.value <- tmp
...
#   } ## for i
# 
#   return(list(max.value = max.value,
#               max.l = max.l,
#               max.r = max.r))
# }
...
# CNVR_202_r1_chr2_p
# main --------------------------------------------------------------------
# minmun number of probs: 50

# path_png = "/sc/orga/projects/haok01a/chengh04/Food_Allergy/code_batch/boundary_refinement/png_V2_rcpp"
...
# i = which(dat_CNVR1$CNVR_ID == "CNVR_550_r1_chr4_q")
...
# refine_step2 = function(Yt, max.iter, max.value, min.len = 5) {
#   
#   n = ncol(Yt)
#   max.iter <- 1000
#   simu.max.values <- rep(NA, max.iter)
#   max.ls <- rep(NA, max.iter)
#   max.rs <- rep(NA, max.iter)
#   for (iter in 1:max.iter) {
#     Yt1 <- Yt[,sample(1:n,n)]
#     # Mcorr1 <- matrix(NA,n,n)  
#     # for (i in 1:n) {
#     #   for (j in 1:n) {
#     #     Mcorr1[i,j] <- cor(Yt1[,i],Yt1[,j])  ## add here
#     #   }
#     # }
#     Mcorr1 = cor(Yt1)
#     S <- Mcorr1[lower.tri(Mcorr1)]
#     n2 <- length(S)
#     
#     max.value1 <- -Inf
#     max.l1 <- 0
...
#         x <- M[lower.tri(M)]
#         n1 <- length(x)
#         if (n1==n2) tmp <- mean(x)/sqrt(1/n1) else tmp <- (mean(x) - (sum(S)-sum(x))/(n2-n1))/sqrt(1/n1+1/(n2-n1))
#         if (i==1 & j==n) tmp <- -Inf
#         if (tmp>max.value1) {
#           max.value1 <- tmp
#           max.l1 <- i
...
#       } ## for j
#     } ## for i
#     simu.max.values[iter] <- max.value1
#     max.ls[iter] <- max.l1
#     max.rs[iter] <- max.r1
#     if (iter %% 100==0) cat("iter =",iter,"\n")
#   } ## for iter
#   
#   p_value <- sum(simu.max.values>=max.value)/max.iter 
#   p_value
# }
