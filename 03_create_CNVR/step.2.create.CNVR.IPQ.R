...
## ensembleCNV only for one CNV calling method
...
read_icnv <- function(file_icnv, col_sel) {
  ...
  dat <- read.table(file = file_icnv, sep = "\t", skip = 17,
...
read_pcnv <- function(file_pcnv, col_sel) {
  ...
  dat <- read.table(file = file_pcnv, sep = "\t", header = FALSE, stringsAsFactors = FALSE, comment.char = "")
...
read_qcnv <- function(file_qcnv, col_sel) {
  ...
  dat <- read.table(file = file_qcnv, sep = "\t", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE, comment.char = "")
  ## Max.log BF
...
option_list <- list(
  make_option(c("-i", "--input"), action = "store", default = NA, type = "character",
              help = "input raw CNV file from IPQ method"),
  make_option(c("-m", "--method"), action = "store", default = NA, type = "character",
              help = "CNV calling method"),
...
pars = c( opt$input, opt$method, opt$snp, opt$output, opt$centromere )
if( any(is.na(pars)) ) {
...
snp <- read.table(file = opt$snp, sep = "\t", header = TRUE, check.names = FALSE, stringsAsFactors = FALSE, comment.char = "") ## for snpName with "#" as comment char
...
cnvs = readRDS( file = opt$input )
cnvs$method = opt$method
cnvs = cnvs[, col_sel]

cat(opt$method, "number of cnvs:", nrow(cnvs), "\n")

cat("CNV preprocessing.\n")
cnv <- preprocess.CNV(cnv = cnvs,
                      snp = snp,
                      centromere = centromere,
                      min.numSNP = 5) ## min.numSNP = 3
...
saveRDS(cnv.create, file = file.path(path_output, paste0("cnvs_step1_create_", opt$method, ".rds")))
saveRDS(cnvr.create, file = file.path(path_output, paste0("cnvrs_step1_create_", opt$method, ".rds")))
...
saveRDS(cnvr.boundary, file = file.path(path_output, paste0("cnvrs_step2_boundary_", opt$method, ".rds")))
